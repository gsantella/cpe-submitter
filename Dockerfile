...
# node:20-slim (Debian/glibc) is required — the pnpm workspace excludes musl
# platform variants, so Alpine cannot be used for build stages.
...
FROM node:20-slim AS sqlite-build
